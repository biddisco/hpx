...
for(std::size_t i=0; i<n; ++i)
    ...
    hpx::when_all(vec).then(
        [&recieved, n](hpx::future<std::vector<hpx::future<std::complex<double>>>> dummy)->void {
        ...
        hpx::cout << "Now Done With Lambda and the last recieved value is " <<recieved[n-1]<< "\n" << hpx::flush;
    }
    ).get();
    return hpx::finalize();
}
...
int main(int argc, char* argv[])
{
    ...
    cmdline.add_options()
        ("nparcels,n", boost::program_options::value<std::size_t>()->default_value(100), "the number of parcels to create")
        ;
