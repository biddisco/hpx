  ...
  public:
    ...
    void done() {
        ...
        if (this->_parent) {
          ...
          char const* fmt1 = "Profile %20s : %2i %5i %9.3f %s %6.3f";
          std::string fmt2 = "Total   " + std::string(20,' ') + " : " + std::string(18,' ') + " %s %6.3f";
          ...
          int last_level = 0;
          hpx::cout << std::string(57+maxlevel*8,'-') << "\n";
          std::for_each(this->_profiles.begin(), this->_profiles.end(),
            [&](std::map<const char *, valtype>::value_type &p) {
              int &level = std::get<1>(p.second);
              level_totals[level] += std::get<0>(p.second);
              if (level < last_level) {
                hpx::cout << std::string(51,' ') << std::string (last_level*8, ' ') << "------\n";
                hpx::cout << (boost::format(fmt2)
                  % std::string (last_level*8, ' ')
