...
/*
                    expected laptop debug build
-------------------------------------------------------------
Solution Validates: avg error less than 1.000000e-13 on all three arrays
-------------------------------------------------------------
Function    Best Rate MB/s  Avg time     Min time     Max time
Copy:            2492.5     0.065030     0.064192     0.066580
Scale:           1588.5     0.102301     0.100723     0.106533
Add:             1395.2     0.175468     0.172014     0.186929
Triad:           1388.6     0.175316     0.172834     0.180143
*/

bool use_pools = false;
...
///////////////////////////////////////////////////////////////////////////////
template <typename Allocator, typename Executor, typename... Targets>
std::vector<std::vector<double> >
run_benchmark(
    std::size_t iterations, std::size_t size,
    std::vector<Executor> execs,
    std::vector<Allocator> allocs,
    Targets... targets)
{
    // Allocate our data
    typedef vector<STREAM_TYPE, Allocator> vector_type;

    std::vector<vector_type> a,b,c;
    for (int i=0; i<execs.size(); ++i) {
        a.push_back(std::move(vector_type(size, allocs[i])));
        b.push_back(std::move(vector_type(size, allocs[i])));
        c.push_back(std::move(vector_type(size, allocs[i])));
    }

    // Creating the policy used in the parallel algorithms
    auto policy = hpx::parallel::execution::par.on(execs[0]);
...
int hpx_main(boost::program_options::variables_map& vm)
{
    ...
    std::cout
        << "-------------------------------------------------------------\n"
        << "Modified RP STREAM bechmark based on\nHPX version: "
            << hpx::build_string() << "\n"
        << "-------------------------------------------------------------\n"
...
#error "The STREAM benchmark currently requires CUDA to run on an accelerator"
...
#endif
    {
        ...
        //
        typedef hpx::threads::executors::pool_executor
                executor_type;
        typedef numa_allocator<STREAM_TYPE, hpx::resource::HPX_MEMBIND_BIND>
                allocator_type;
        //
        ...
        // For each numa domain, get the pool executor
        for (const hpx::resource::numa_domain &d : rp.numa_domains())
        {
            std::string pool_name = "numa-"+std::to_string(d.id());
            executor_type exec    = executor_type(pool_name);
            allocator_type alloc  = allocator_type(d.get_hwloc_bitmap());
        ...
        timing =
            run_benchmark<allocator_type, executor_type>(
                iterations, vector_size, executors, allocators);
    }
...
int main(int argc, char** argv)
{
    ...
    // For each numa domain, create a pool
    for (const hpx::resource::numa_domain &d : rp.numa_domains())
    {
        // create a pool with the default scheduler
        std::string pool_name = "numa-"+std::to_string(d.id());
        rp.create_thread_pool(pool_name,
            hpx::resource::scheduling_policy::local_priority_fifo);
        // add all the cores on this numa domain to the pool
        rp.add_resource(d, pool_name);
    }
    // we need to tell the RP that the "default" pool is going to be
    // the "numa-0" pool
    rp.set_default_pool_name("numa-0");
