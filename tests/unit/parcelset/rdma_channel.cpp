...
//
#include <plugins/parcelport/verbs/rdma_object.hpp>
//
#include <cstddef>
#include <iostream>
#include <string>
...
const int down = 1;
//
std::array<rdma_object<dummy_data>, 2> recv;
std::array<rdma_object<dummy_data>, 2> send;
//
void test_rdma_1(hpx::id_type loc)
{
    ...
    if (rank > 0)
    {
        recv[up] = hpx::find_from_basename<rdma_object<dummy_data>>(down_name, rank - 1);
        send[up] = hpx::rdma::make_rdma_object<dummy_data>();
        hpx::register_with_basename(down_name, send[up], rank);
    }
    if (rank < num - 1)
    {
        recv[down] = hpx::find_from_basename<channel_type>(up_name, rank + 1);
        send[down] = hpx::rdma::make_rdma_object<dummy_data>();
