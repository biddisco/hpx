    ...
    void sender::async_write_impl()
    {
        ...
        for (auto &c : buffer_.chunks_)
        {
            if (c.type_ == serialization::chunk_type_pointer)
            {
                ...
                // @TODO : get rid of this heap allocation by using region by ref
                libfabric_memory_region *zero_copy_region = new libfabric_memory_region(
                    domain_, c.data_.cpos_, c.size_, 16); // alignment of 16 bytes
        ...
        region_list_[1] = {
            message_region_->get_address(), message_region_->get_message_length_aligned(16) };
...
/*
        ...
        std::fill(message_region_->get_address(),
                  ...
                  255);
*/
        // cleanup the message region : the header region is reused on the next send,
        ...
        {
            // this calls delete on user regions
