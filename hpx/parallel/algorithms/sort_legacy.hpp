...
#if !defined(HPX_PARALLEL_ALGORITHM_SORT_LEGACY)
#define HPX_PARALLEL_ALGORITHM_SORT_LEGACY
...
namespace hpx { namespace parallel { HPX_INLINE_NAMESPACE(v1)
{
    ...
    namespace detail
    {
        ...
        template <typename ExPolicy, typename RandomIt, typename Compare>
        hpx::future<RandomIt>
        parallel_sort_legacy_async(ExPolicy && policy, RandomIt first, RandomIt last,
            Compare comp)
        {
        ...
        struct sort_legacy : public detail::algorithm<sort_legacy<RandomIt>, RandomIt>
        {
            sort_legacy()
              : sort_legacy::algorithm("sort")
            {}
                ...
                Compare && comp, Proj && proj)
            {
                ...
                return util::detail::algorithm_result<ExPolicy, RandomIt>::get(
                    parallel_sort_legacy_async(std::forward<ExPolicy>(policy),
                        first, last,
                        util::compare_projected<Compare, Proj>(
                            std::forward<Compare>(comp),
                            std::forward<Proj>(proj)
    ...
    sort_legacy(ExPolicy && policy, RandomIt first, RandomIt last,
        Compare && comp = Compare(), Proj && proj = Proj())
    {
        ...
        return detail::sort_legacy<RandomIt>().call(
