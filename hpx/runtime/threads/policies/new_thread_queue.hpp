    ...
    protected:
            ...
            std::unique_lock<mutex_type> &lk, bool steal = false)
        {
            ...
            if (HPX_LIKELY(max_count_)) {
                std::size_t count = thread_map_size();
        ...
        {
//            std::cout << "add to thead map " << std::hex << thrd << std::endl;
            // add the new entry to the map of all threads
        ...
        {
//            std::cout << "recycle_thread " << std::hex << thrd << std::endl;
...
//                    std::cout << "thread_map_.erase 1 " << std::hex << tid.get() << std::endl;
                ...
                thread_data* todelete = nullptr;
                while (delete_count && terminated_items_.pop(todelete))
                {
                    thread_id_type tid(todelete);
                    --terminated_items_count_;
//                    std::cout << "thread_map_.erase 2 " << std::hex << tid.get() << std::endl;
                    HPX_ASSERT(todelete == thread_map_.erase(reinterpret_cast<std::uintptr_t>(tid.get())));
                    recycle_thread(tid);
                    --thread_map_count_;
                    HPX_ASSERT(thread_map_count_ >= 0);
    ...
    public:
            ...
            thread_state_enum initial_state, bool run_now, error_code& ec)
        {
            ...
            if (run_now)
            {
                ...
                // suspended.
                {
                    ...
                    add_to_thread_map(thrd, initial_state);
            ...
            bool allow_stealing = false, bool steal = false) HPX_HOT
        {
            ...
            {
//                std::cout << "get next thead (work pop)" << std::hex << thrd << std::endl;
                --work_items_count_;
                return true;
            }
#endif
...
#else
//            std::cout << "schedule thead (work push) " << std::hex << thrd << std::endl;
        ...
        {
//            std::cout << "destroy thead " << std::hex << thrd << std::endl;
            HPX_ASSERT(&thrd->get_queue<new_thread_queue>() == this);
...
//                junction::DefaultQSBR.flush();
    ...
    util::internal_allocator<threads::thread_data> new_thread_queue<Mutex,
        PendingQueuing, StagedQueuing, TerminatedQueuing>::thread_alloc_;

    template <typename Mutex, typename PendingQueuing, typename StagedQueuing,
        typename TerminatedQueuing>
    util::internal_allocator<typename new_thread_queue<Mutex, PendingQueuing,
            StagedQueuing, TerminatedQueuing>::task_description>
        new_thread_queue<Mutex, PendingQueuing, StagedQueuing,
