...
namespace hpx { namespace threads { namespace policies
{
    ...
    struct alignas(64) queue_holder_thread
    {
        ...
        // ----------------------------------------------------------------
        queue_holder_thread(
                QueueType *hp_queue, QueueType *np_queue, QueueType *lp_queue, int owner, int id)
            : hp_queue_(hp_queue)
            , np_queue_(np_queue)
            , lp_queue_(lp_queue)
            , owner_mask_(owner)
...
//            }
            ...
            if (delete_all) {
                ...
                while (terminated_items_.pop(todelete))
                {
                    ...
                    debug("deallocate", queue_index_,
                          np_queue_->new_tasks_count_,
                          np_queue_->work_items_count_,
                          nullptr);
                }
            }
            else {
                           ...
                           thread_state_enum state, bool run_now, error_code& ec)
        {
                 ...
                 data.priority == thread_priority_boost))
            {
                ...
                return hp_queue_->create_thread(data, thrd, state, run_now, ec);
            }

            if (lp_queue_ &&
                (data.priority == thread_priority_low))
            {
                return lp_queue_->create_thread(data, thrd, state, run_now, ec);
            }

            // normal priority + anything unassigned above (no hp queues etc)
            np_queue_->create_thread(data, thrd, state, run_now, ec);
...
//            HPX_ASSERT(&thrd->get_queue<queue_holder_thread>() == this);
            ...
            bool running, std::int64_t& idle_loop_count, std::size_t& added, bool steal)
        {
            bool result = true;
            if (owns_hp_queue()) {
                result = hp_queue_->wait_or_add_new(running, idle_loop_count, added)
                        && result;
            }

            if (owns_np_queue()) {
                result = np_queue_->wait_or_add_new(running, idle_loop_count, added)
                        && result;
            }

            if (owns_lp_queue()) {
                result = lp_queue_->wait_or_add_new(running, idle_loop_count, added)
                        && result;
            }
            return result;
        }
        ...
        void abort_all_suspended_threads()
        {
                 ...
                 it != end; ++it)
            {
                if ((*it)->get_state().state() == suspended)
                {
                    ...
                    np_queue_->schedule_thread((*it).get(), true);
                }
            }
        }
