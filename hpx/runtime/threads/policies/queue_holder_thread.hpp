...
#include <libs/debug/include/hpx/debug/locks.hpp>
...
namespace hpx { namespace threads { namespace policies {
    ...
    struct queue_holder_thread
    {
        ...
        bool cleanup_terminated(bool delete_all = false)
        {
            ...
            typename debug::lock<mutex_type, false>::scoped_lock lk(
                        thread_map_mtx_.data_);
        ...
        // ----------------------------------------------------------------
        bool cleanup_terminated_lock(bool delete_all = false)
        {
            if (terminated_items_count_.data_.load(std::memory_order_relaxed) == 0)
                return true;
        ...
        {
            typename debug::lock<mutex_type, false>::unique_lock lk(
                thread_map_mtx_.data_);
            ...
            if (/*HPX_UNLIKELY*/(!p.second)) {
                ...
                threads::thread_data *td = tid2.get();
        ...
        void abort_all_suspended_threads()
        {
                 ...
                 it != end; ++it)
            {
                if ((*it)->get_state().state() == suspended)
                {
                    ...
                    np_queue_->schedule_work((*it).get(), true);
                }
            }
        }
