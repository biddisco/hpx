...
namespace policies {
    ...
    public:
        ...
        typedef new_thread_queue<> thread_queue_type;
...
//            LOG_CUSTOM_MSG("cleanup_terminated - global version");
            ...
            threads::thread_data* thrd, std::int64_t& busy_count) override
        {
            ...
            thrd->get_queue<queue_holder_thread<new_thread_queue<>>>().destroy_thread(thrd, busy_count);
        }
        ...
        void on_start_thread(std::size_t thread_num) override
        {
            ...
            queue_holder_thread<new_thread_queue<>> *thread_holder =
                    new queue_holder_thread<new_thread_queue<>>(hp_queue, np_queue, lp_queue, owner_mask, thread_counter);
