...
namespace hpx { namespace threads { namespace policies
{
    ...
    public:
                            ...
                            bool stealing)
        {
            ...
            while (add_count-- && addfrom->new_task_items_.pop(task, stealing))
            {
                ...
                holder_->create_thread_object(thrd, data, state);
                holder_->add_to_thread_map(thrd);
    ...
    public:
            ...
            thread_state_enum initial_state, bool run_now, error_code& ec)
        {
            ...
            {
                threads::thread_id_type thrd;
                holder_->create_thread_object(thrd, data, initial_state);
                holder_->add_to_thread_map(thrd);

                // push the new thread in the pending queue thread
                if (initial_state == pending)
                    schedule_work(thrd.get(), false);
