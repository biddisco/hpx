...
int main(int argc, char* argv[])
{
    ...
    rp.create_thread_pool("default",
        [](hpx::threads::thread_pool_init_parameters init,
            ...
            custom_sched::init_parameter_type scheduler_init(
                init.num_threads_,
            ...
            std::unique_ptr<custom_sched> scheduler(
                new custom_sched(scheduler_init));

            init.mode_ = scheduler_mode(scheduler_mode::delay_exit);

            std::unique_ptr<hpx::threads::thread_pool_base> pool(
                new hpx::threads::detail::scheduled_thread_pool<
                    custom_sched>(std::move(scheduler), init));
            return pool;
        });
